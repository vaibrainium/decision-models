...
def get_chronometric_data(data: Dict[str, np.ndarray], positive_direction: str = "right") -> Tuple[np.ndarray, ...]:
    ...
    return tuple(map(np.array, zip(*sorted(results))))
...
class CUDADDMSimulator:
    ...
    def __init__(self, leak: bool = True, time_dependence: bool = True, device: Optional[str] = None):
        # Set device
    ...
    def simulate_trials(self, stimulus: Union[np.ndarray, torch.Tensor]) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
...
class OptimizedDecisionModel:
    ...
    def __init__(self, model_name: str = "DDM", enable_leak: bool = True, enable_time_dependence: bool = True, device: Optional[str] = None, likelihood_params: Optional[Dict] = None):
    ...
    def fit(self, data: Dict[str, np.ndarray], stimulus: np.ndarray, max_iterations: int = 100, n_reps: int = 5, seed: int = 42, l1_weight: float = 0.01, verbose: bool = True) -> Dict:
        ...
        if verbose:
            ...
            for name, value in zip(self._param_bounds.keys(), result.x):
                logger.info(f"  {name} = {value:.4f}")

        # Return comprehensive results
        return {"success": result.success, "parameters": dict(zip(self._param_bounds.keys(), result.x)), "likelihood": result.fun, "n_iterations": result.nit, "optimization_result": result}
